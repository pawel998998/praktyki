...
with open("e_dziennik/oceny.html", encoding="utf-8") as template:
   oceny = template.read()

with open("e_dziennik/sprawdziany.html", encoding="utf-8") as template:
...
class main(object):
   ...
   def plan_lekcji_admin_edycja(self, id):
      ...
      prompt +="<td><button type='submit'>Zapisz</button></td></table></form>"
      if cherrypy.session.get('logged_in'):
         return plan_lekcji.replace("{table}", prompt)
      else:
         raise cherrypy.HTTPRedirect('/index_login_page')
   ...
   def oceny(self):
      ...
      mycursor.execute(f"select ocena, typ, data, id_przedmiot from oceny where id_ucznia = '{id_query_usera[0][0]}'")
      ...
      for i in range(len(oceny_db)):
         ...
         prompt += f"<td>{oceny_db[i][3]}</td>"
         prompt += f"<td>{oceny_db[i][1]}</td></tr>"
      prompt += "</table>"

      return oceny.replace("{table}", prompt)
